...
def load_all_csv():
    ...
    # Try multiple possible locations (remove duplicates)
    possible_paths = [
        RAW_DIR / 'all.csv',
        Path('all.csv'),  # Fallback to root
    ]
    ...
    if all_csv is None:
        print(f"  [INFO] all.csv not found in any of these locations:")
        for path in unique_paths:
            print(f"    - {path}")
        print(f"  [INFO] Data collection will be skipped.")
        print(f"  [INFO] Other scripts will use existing processed data from data/processed/")
        return None
...
def apply_university_mappings(df):
    ...
    # Try multiple possible locations for mapping file
    mapping_file = None
    ...
    if mapping_file is None:
        raise FileNotFoundError("Could not find university_mappings_from_qs.csv")
...
def main():
    ...
    df = load_all_csv()
    if df is None:
        print("\n[INFO] Data collection step skipped (all.csv not available).")
        print("       This is expected if you already have processed data.")
        print("       Analysis will continue using data/processed/final_combined_dataset.csv")
        return None
    ...
    qs_df = None
    # Try multiple patterns to find QS rankings file
    qs_files = (list(RAW_DIR.glob('*QS*.csv')) + 
                list(RAW_DIR.glob('*ranking*.csv')) + 
                list(RAW_DIR.glob('qs_rankings*.csv')))
    if qs_files:
        try:
            qs_file = qs_files[0]
