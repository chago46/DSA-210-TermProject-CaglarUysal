...
def create_qs_based_mappings():
    ...
    qs_file_paths = [
        Path('data/raw/qs_rankings_2025.csv'),
        Path('data/raw/QS World University Rankings 2025 (Top global universities).csv'),
        Path('qs_rankings_2025.csv')  # Fallback
    ]
    
    qs_df = None
    for qs_path in qs_file_paths:
        if qs_path.exists():
            try:
                qs_df = pd.read_csv(qs_path, low_memory=False, encoding='utf-8')
                print(f"  [OK] Loaded QS Rankings from {qs_path}")
                break
            except UnicodeDecodeError:
                try:
                    qs_df = pd.read_csv(qs_path, low_memory=False, encoding='latin-1')
                    print(f"  [OK] Loaded QS Rankings from {qs_path} (latin-1)")
                    break
                except:
                    try:
                        qs_df = pd.read_csv(qs_path, low_memory=False, encoding='cp1252')
                        print(f"  [OK] Loaded QS Rankings from {qs_path} (cp1252)")
                        break
                    except Exception as e:
                        print(f"  [WARN] Could not read {qs_path}: {e}")
                        continue
    
    if qs_df is None:
        raise FileNotFoundError("Could not find QS Rankings file. Expected: data/raw/qs_rankings_2025.csv")
    ...
    csv_file = mappings_dir / 'university_mappings_from_qs.csv'
    ...
    # Save text file
    txt_file = mappings_dir / 'university_mappings_from_qs.txt'
