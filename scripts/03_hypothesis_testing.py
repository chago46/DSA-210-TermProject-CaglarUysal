...
def load_data():
    ...
    possible_paths = [
        ...
        PROCESSED_DIR / 'admissions_merged.csv',
        Path('final_combined_dataset.csv'),  # Fallback
    ...
    if csv_files:
        ...
        raise FileNotFoundError("No data file found. Please run scripts/01_data_collection.py first.")
    ...
    if not results:
        ...
        if (sop_col and sop_col in df.columns) or (lor_col and lor_col in df.columns):
            ...
            if sop_count == 0 and lor_count == 0:
                ...
                ax.text(0.5, 0.5, 'SOP/LOR data not available\nin this dataset', 
                       ha='center', va='center', fontsize=14, transform=ax.transAxes)
                ax.set_title('Hypothesis 4: SOP and LOR Impact\n(Data Not Available)')
                ax.axis('off')
                plt.tight_layout()
                plt.savefig(OUTPUT_DIR / 'hypothesis_4.png', dpi=300, bbox_inches='tight')
                plt.close()
                print("[INFO] Placeholder visualization saved for Hypothesis 4")
            else:
    ...
    if qs_cols:
        ...
        for qs_col in key_qs_cols:
            if qs_col in df.columns:
                ...
                if len(data) > 1:
                    try:
                        # Convert to numeric if it's an object type (like qs_RANK_2025)
                        qs_series = pd.to_numeric(data[qs_col], errors='coerce')
                        target_series = pd.to_numeric(data[target_col], errors='coerce')
                        valid_data = pd.DataFrame({qs_col: qs_series, target_col: target_series}).dropna()
                        
                        if len(valid_data) > 1:
                            corr, p_val = pearsonr(valid_data[qs_col], valid_data[target_col])
                            ...
                            print(f"  [WARN] Could not calculate correlation for {qs_col}: insufficient numeric data after conversion")
                    except Exception as e:
