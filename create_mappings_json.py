...
def create_mappings_json():
    ...
    print("\nLoading mappings...")
    try:
        mapping_df = pd.read_csv('university_mappings_from_qs.csv')
        print(f"  [OK] Loaded {len(mapping_df)} mappings from CSV")
    except FileNotFoundError:
        print("  [ERROR] university_mappings_from_qs.csv not found")
        print("  Running mapping script first...")
        import subprocess
        subprocess.run(['python', 'map_universities_from_qs.py'], check=False)
        try:
            mapping_df = pd.read_csv('university_mappings_from_qs.csv')
            print(f"  [OK] Loaded {len(mapping_df)} mappings")
        except:
            print("  [ERROR] Could not create mappings. Please run map_universities_from_qs.py first.")
            return
    ...
    # Save simple JSON
    simple_file = 'university_mappings_simple.json'
    ...
    # Save detailed JSON
    detailed_file = 'university_mappings_detailed.json'
    ...
    # Save grouped JSON
    grouped_file = 'university_mappings_grouped.json'
